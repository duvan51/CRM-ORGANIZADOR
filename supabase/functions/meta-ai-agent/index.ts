...
serve(async (req) => {
    ...
    try {
        ...
        let configQuery = supabaseServer.from('ai_agent_config').select('*')
        if (isTest && clinicId) {
            configQuery = configQuery.eq('clinic_id', clinicId)
        } else if (phoneId) {
            configQuery = configQuery.eq('phone_id', phoneId).eq('is_active', true)
        } else {
            return new Response('Missing identification', { status: 200, headers: corsHeaders })
        }
        ...
        if (!config || configError) {
            return new Response(JSON.stringify({
                error: `Configuración de IA no encontrada para la clínica: ${clinicId}. Por favor, haz clic en "Guardar Config IA" en el panel.`
            }), {
                status: 200,
