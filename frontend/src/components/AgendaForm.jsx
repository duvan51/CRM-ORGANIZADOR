        ...
        const fetchConfig = async () => {
            try {
                const [sRes, hRes, bRes, hsRes] = await Promise.all([
                    ...
                    fetch(`http://localhost:8000/agendas/${agendaId}/bloqueos`, { headers: { "Authorization": `Bearer ${token}` } }),
                    fetch(`http://localhost:8000/agendas/${agendaId}/horarios-servicios`, { headers: { "Authorization": `Bearer ${token}` } })
                ]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = initialData
                ? `http://localhost:8000/citas/${initialData.id}`
