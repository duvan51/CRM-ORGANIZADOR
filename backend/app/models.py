...
class Agenda(Base):
    ...
    agenda_services = relationship("AgendaService", back_populates="agenda")
    horarios_servicios = relationship("HorarioServicio", back_populates="agenda")
...
class GlobalService(Base):
    ...
    concurrency = Column(Integer, default=1) # Maximo de este servicio al mismo tiempo
    color = Column(String, default="#3b82f6")
    image_url = Column(String, nullable=True)
    descripcion = Column(Text, nullable=True)

    agendas = relationship("AgendaService", back_populates="service")
    horarios = relationship("HorarioServicio", back_populates="service")
...
class Bloqueo(Base):
    ...
    service_id = Column(Integer, ForeignKey("global_services.id"), nullable=True) # Bloqueo para un servicio puntual
    tipo = Column(Integer, default=1) # 1: Bloqueo, 2: Habilitación (Excepción)
...
class HorarioServicio(Base):
    __tablename__ = "horarios_servicios"
    id = Column(Integer, primary_key=True, index=True)
    agenda_id = Column(Integer, ForeignKey("agendas.id"))
    service_id = Column(Integer, ForeignKey("global_services.id"))
    dia_semana = Column(Integer) # 0-6 (L-D)
    ...
    agenda = relationship("Agenda", back_populates="horarios_servicios")
    service = relationship("GlobalService", back_populates="horarios")
